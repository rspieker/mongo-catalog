...
function versionsMatch(meta1: MetaData, meta2: MetaData): boolean {
    ...
    const completed1 = meta1.catalog.filter(c => c.completed && c.resultChecksum);
    const completed2 = meta2.catalog.filter(c => c.completed && c.resultChecksum);
...
function binarySearchPriority(
    ...
    if (!result.has(latestVersion.plan.version)) {
        result.set(latestVersion.plan.version, { version: latestVersion, priority });
        priority++;
    }
    if (startIdx !== endIdx && !result.has(earliestVersion.plan.version)) {
        result.set(earliestVersion.plan.version, { version: earliestVersion, priority });
        priority++;
    }
    ...
    const outputsMatch = versionsMatch(latestVersion.meta, earliestVersion.meta);
    
    if (outputsMatch) {
        // All versions in between have same output - mark them with low priority
        for (let i = startIdx + 1; i < endIdx; i++) {
            if (!result.has(versions[i].plan.version)) {
                result.set(versions[i].plan.version, { version: versions[i], priority: 1000 });
            }
        }
        return priority;
    } else {
        ...
        if (!result.has(midVersion.plan.version)) {
            result.set(midVersion.plan.version, { version: midVersion, priority });
            priority++;
        }
        
        // Recurse on both halves
        priority = binarySearchPriority(versions, startIdx, midIdx, priority, result);
        priority = binarySearchPriority(versions, midIdx, endIdx, priority, result);
...
>(catalogFile)
    .then(async (catalog) => {
        ...
        for (const metaFile of metaFiles) {
            ...
            if (!plansAreEqual(plan, existingPlan)) {
                ...
                console.log(`Updated plan for ${version}: ${pendingCatalogs.length} pending catalogs`);
            } else {
                console.log(`Skipped saving plan for ${version} - no changes`);
            }

            allVersions.push({ plan, version: new Version(version), meta, versionDir });
        }
    ...
    .then((allVersions) => {
        ...
        const withPending = allVersions.filter((v) => v.plan.catalogs.length > 0);
        ...
        const byMinor = withPending.reduce((carry, item) => {
        ...
        const prioritizedVersions: Array<{ version: VersionWithMeta; priority: number }> = [];
        ...
        for (const [minorKey, versions] of Object.entries(byMinor)) {
            ...
            const sortedVersions = versions.sort((a, b) => 
                (a.version.patch || 0) - (b.version.patch || 0)
            );

            console.log(`\nProcessing ${minorKey} (sorted earliest→latest): ${sortedVersions.map(v => v.plan.version).join(', ')}`);

            // Apply binary search prioritization
            const priorityMap = new Map<string, { version: VersionWithMeta; priority: number }>();
            binarySearchPriority(sortedVersions, 0, sortedVersions.length - 1, 1, priorityMap);
        ...
        const prioritiesByMinor: { [key: string]: Array<{ version: VersionWithMeta; priority: number }> } = {};
